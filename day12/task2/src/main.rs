...
struct Path {
    cave: usize,
    visited2: bool, // has some node visited 2 times
    prev: Option<Rc<Path>>,
}
        ...
        if path.cave == end {
            ...
            qju.extend(
                graph.caves[path.cave]
                    ...
                    .cloned()
                    .filter_map(|idx| {
                        let visited = graph.caves[idx].small && path.visited(idx);
                        if idx == start || (visited && path.visited2) {
                            None
                        } else {
                            Some(Rc::new(Path {
                                cave: idx,
                                visited2: visited || path.visited2,
